...
## Две разные прошивки + тест Dell-FW (2026-08-23, ночь)
- Пользователь подтвердил: ЭТОТ ноутбук с ЭТОЙ картой декодировал под Win10/11 x64
  через фильтр «Broadcom Video Decoder» (bcmDIL.dll 3.17 + BRCMHD64.sys из Dell
  R286525). => Железо декодирует. Гипотеза «чип не умеет» ЗАКРЫТА.
- В пакете Dell лежит ДРУГОЙ билд прошивки: 864276 B, но md5 36611b7b…,
  версия **1.23.2.33 (2009/07/08)** против нашей **1.54.0.0 (2010/11/30)**
  (md5 213e2424…) из mainline-kmod. Различаются 661366 байт (~76%).
- Тест на ноутбуке с /lib/firmware/bcm70015fw.bin = Dell-прошивка:
  поведение АБСОЛЮТНО идентично (mask=0, тот же y_err 0x800/0x2000,
  те же Ycnt 40/38440, FETCH TIMEOUT). => Версия FW не влияет на данный отказ.
- Также исключены: валидность входного файла (test2.h264 проверен ffprobe/ffmpeg —
  Constrained Baseline 320x240, декодируется ffmpeg'ом без ошибок; плюс свежий
  сгенерированный gen.h264 240 кадров), startCodeSz=3 vs 4 (cdt3/cdt4).

### Вывод
Отказ воспроизводится с двумя разными билдами FW и разными входами =>
разница между рабочим Windows-стеком и нашим Linux-стеком — в ХОСТОВОЙ части:
(а) инициализация железа в BRCMHD64.sys vs crystalhd_flea_ddr.c/kmod
    (DDR-тайминги, скраббер, арбитр, клоки, power domains), либо
(б) байты PES-пакетов/команд bcmDIL 3.17 vs наша либа 3.22.
Файлы для анализа: /tmp/dellx/R286525/program files/Broadcom/
Broadcom CrystalHD Decoder/{bcmDIL.dll (PE32, 237K), 64-bit/BRCMHD64.sys (150K)}.
Следующий шаг: дизассемблировать init-последовательность BRCMHD64.sys
(register trace) и сравнить с нашим crystalhd_flea_download_fw/ddr_init;
затем сверить DtsPESConverter 3.17 vs 3.22 побайтно.

## Эксперименты на живой Windows (2026-08-25, Win11 22H2 на ноуте)
Стек: Dell BRCMHD64.sys (байт-в-байт тот же), bcmDIL 3.6.1 из MSI, K-Lite/MPC-HC.
Инструменты: PowerShell P/Invoke в bcmDIL.dll (C:\brcm\*.ps1 на ноуте,
в vista-kit на десктопе). Ключи: DtsDeviceOpen требует ключ реестра
HKLM\SOFTWARE\Broadcom\MediaPC\CrystalHD (без него -1); DLL ищет прошивку
по именам bcm70015fwbd.bin/bcm70015fw.bin рядом с собой; MSI на Win11
падает на MsiProcessDrivers (ставить вручную/распаковкой).

Результаты:
1. Официальный Windows-стек ВОСПРОИЗВОДИТ ОТКАЗ: граф собирается, декодер
   отдаёт формат YUY2 1920x1080, но 0 кадров; MPC-Hc crash — ACCESS VIOLATION
   в bcmDIL!DtsProcOutput→DtsChkYUVSizes (скрин у пользователя).
2. HVCI/VBS выключены — изоляция ядра не при чём. Пересадка платы/ОЗУ не помогла.
3. DRAM чипа полностью исправна (walk-тест 0x100000..0x3800000 OK; 0x0-0x10000
   «фейл» закономерен — живой образ FW).
4. FW жива: команды обрабатываются, очередь слотов идентична Linux
   (INIT→CH_OPEN→INPUT_PARAMS→ACTIVATE→START_VIDEO, rsp нули — by design).
5. НО: на Windows TX DMA вообще не стартует — после ProcInput (sts=0)
   TX info block @0xd3300 и кольцо @0x210000 не меняются, сигнатура битстрима
   в DDR (0x0-0x400000) не найдена. FLL@0xd3420=0x18 (FW разрешил приём).
   Вероятная причина — несовместимость драйвера 2009 г. с DMA-стеком Win11.
   Это ОТДЕЛЬНЫЙ сбой, не наш Linux-баг (на Linux TX работает).

### Состояние главного вопроса (Linux: вход ест, кадров нет)
- Исключены: версия FW (обе), входной файл, startCodeSz, HVCI, DDR-память чипа,
  пересадка железа.
- Осталось проверить на Linux (когда вернём SSD): **попадают ли байты битстрима
  в кольцо DRAM реально** (TX done мы видели, но байты не проверяли!), и шевелятся
  ли указатели RAVE-парсера (CDB/ITB). Если байты на месте и парсер молчит —
  проблема в FW/декодере ядра → вероятно железо (ARC/видеопайп), и карта
  под замену/в другую машину.
